    ...
    private Map<String, VelocityContext> analyzeData(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            if (tableInfo.isConvert()) {
                tableInfo.getImportPackages().remove("com.baomidou.mybatisplus.annotations.TableField");
                // 表注解
                tableInfo.setImportPackages("com.yihaodian.scm.core.bean.TableName");
            }
            ...
            if (config.getStrategyConfig().isEntityBooleanColumnRemoveIsPrefix()) {
                tableInfo.getFields().stream().filter(field -> field.getPropertyType().equalsIgnoreCase("boolean"))
                        .filter(field -> field.getPropertyName().startsWith("is")).forEachOrdered(field -> {
            ...
            tableFields.addAll(tableInfo.getFields());
            tableFields.addAll(tableInfo.getCommonFields());
            Map<String, GenColumn> pageColumn = ((List<GenColumn>) injectionConfig.getMap().get("pageColumn")).stream()
                    .collect(Collectors.toMap(GenColumn::getKey, o -> o));
            List<GenColumn> searchColumn = new ArrayList<>();
            List<GenColumn> editColumn = new ArrayList<>();
            List<GenColumn> tableColumn = new ArrayList<>();
            for (TableField field : tableFields) {
                GenColumn temp = pageColumn.get(field.getPropertyName());
                if (temp == null) {
                    continue;
                }
                if (temp.isEdit()) {
                ...
                if (temp.isSearch()) {
                    searchColumn.add(temp);
                }
                if (temp.isTable()) {
                    tableColumn.add(temp);
                }
            }
            ctx.put("searchColumn", searchColumn);
            ctx.put("tableColumn", tableColumn);
            ctx.put("editColumn", editColumn);
            ctx.put("entityNameFirstLower", StringUtils.firstCharToLower(tableInfo.getEntityName()));
            Map<Object, Object> param = new HashMap<>();
            for (Object key : ctx.getKeys()) {
                param.put(key, ctx.get(key.toString()));
            }
    ...
    private void batchOutput(String entityName, VelocityContext context) {
        try {
            ...
            String entityFile = String.format(
                    (pathInfo.get(ConstVal.ENTITY_PATH) + File.separator + "%s" + this.suffixJavaOrKt()), entityName);
            String mapperFile = String.format((pathInfo.get(ConstVal.MAPPER_PATH) + File.separator
                    + tableInfo.getMapperName() + this.suffixJavaOrKt()), entityName);
            ...
            String serviceFile = String.format((pathInfo.get(ConstVal.SERIVCE_PATH) + File.separator
                    + tableInfo.getServiceName() + this.suffixJavaOrKt()), entityName);
            String implFile = String.format((pathInfo.get(ConstVal.SERVICEIMPL_PATH) + File.separator
                    + tableInfo.getServiceImplName() + this.suffixJavaOrKt()), entityName);
            String controllerFile = String.format((pathInfo.get(ConstVal.CONTROLLER_PATH) + File.separator
                    + tableInfo.getControllerName() + this.suffixJavaOrKt()), entityName);
